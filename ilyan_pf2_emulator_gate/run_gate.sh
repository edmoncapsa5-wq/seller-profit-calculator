...
cat \
  ...
  ilyan_pf2_emulator_gate/rom.b64.part01 \
...
EXPECTED_SHA='b2fbb61129e8fc9bdae6a508d8789b2f15a4287e179c5b80b9451469775c306a'
...
native_shot 00_boot; window_shot 00_boot_x11; kill -0 "$MGBA_PID"
...
key_down a; key_tap x; key_up a; sleep 0.07; native_shot 17_dalum_early; sleep 0.065; native_shot 18_dalum_active
key_tap Return; sleep 0.05; key_tap x; sleep 0.08; native_shot 19_debug_contact
sleep 5; kill -0 "$MGBA_PID"; native_shot 20_soak; window_shot 20_soak_x11
...
assert len(files) >= 32, f'only {len(files)} native screenshots'
rows=[]; thumbs={}
for p in files:
    im=Image.open(p).convert('RGB'); thumb=im.resize((120,80),Image.Resampling.NEAREST); thumbs[p.name]=thumb
...
assert len(set(r['sha256'] for r in rows)) >= 13, 'too few distinct emulated frames'
boot_full=Image.open('screenshots_native/00_boot.png').convert('RGB')
def bright_count(box, threshold=150):
    return sum((299*r+587*g+114*b)/1000 > threshold for r,g,b in boot_full.crop(box).getdata())
ui_bright={'player_panel':bright_count((3,3,116,29)),'enemy_panel':bright_count((134,3,237,20))}
assert ui_bright['player_panel'] >= 18 and ui_bright['enemy_panel'] >= 12, ui_bright
boot=thumbs['00_boot.png']
def mad(a,b): return sum(ImageStat.Stat(ImageChops.difference(a,b)).mean)/3
probe=['02_cruise.png','04_reverse.png','07_quick_early.png','10_attack1.png','12_attack3.png','14_guard.png','17_dalum_early.png','18_dalum_active.png','19_debug_contact.png','20_soak.png']
change={n:round(mad(boot,thumbs[n]),3) for n in probe}
assert max(change.values())>1.5, change
assert mad(thumbs['17_dalum_early.png'],thumbs['18_dalum_active.png'])>0.20, 'Dalum active frame did not visibly change'
...
Path('results/ui_contrast_gate.json').write_text(json.dumps(ui_bright,indent=2))
print(json.dumps({'render_change':change,'ui_bright':ui_bright},indent=2))
PY
...
fi
{
  ...
  echo 'UI_CONTRAST_GATE=PASS'
  echo 'NOTE=This certifies boot/render/input-path/liveness and HUD contrast, not subjective controller feel.'
